...
from src.utils.logger import get_logger, log_scraping_session
from src.utils.exceptions import handle_exceptions, TwitchTrackerException
...
Exemples d'usage:
  ...
  python main.py --status                           # Affiche le statut
    ...
    parser.add_argument(
        '--scraper',
        nargs='*',
        choices=['games', 'events', 'streamers', 'all'],
        default=['all'],
...
def main():
    ...
    try:
        ...
        if 'all' in scrapers_to_run:
            scrapers_to_run = ['games', 'events', 'streamers']
